...
Project Description: 
The aim of the project is to study a large-scale randomized controlled trial that  provided 
a universal cash transfer of USD 416 (MWK 701,000) to around 70,000 individuals over the 
course of 12 months in Malawi. Rollout was randomized across Group Village Headman geographic 
clusters (GVHs). The study leverages the randomization in the timing of the receipt of the cash transfer. 

Brief Description of Data: 
Study uses a panel data set which covers information on household outcomes across 8 waves. 
This includes two baseline surveys and six follow-up surveys.  
...
/*
NOTE: There are 223 observations with missing values in gender_num and these are missing 
either because they were missing in the original gender_string variable or because the mode 
function used to created gender_cleaned variable could not pick one value. In the latter case, 
this is mainly because there are an equal number of male/female values within an ID. For 
instance, for ID = 1735, there three males and three females recorded under the original 
gender_string variable so the mode function could not pick one value.  
*/
